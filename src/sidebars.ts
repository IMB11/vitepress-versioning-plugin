...
function replaceLinksRecursive(
  sidebar: VVP.SidebarItem[],
  config: VVP.SidebarConfig,
  version: VVP.Version
    ...
    if (Array.isArray(sidebar)) {
      ...
      Object.keys(multiSidebar).forEach((key) => {
        multiSidebar[key] = replaceLinksRecursive(
          multiSidebar[key] as VVP.SidebarItem[],
          config,
          (locale === "root" ? "" : `${locale}/`) + version
        );
      });
...
export function generateVersionSidebars(
  config: VVP.SidebarConfig | false,
  dirname: string,
  versions: VVP.Version[],
  locales: string[]
